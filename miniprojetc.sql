create database mini_project08;
use mini_project08;
...
select guests.guest_name ,
		count(bookings.room_id) as total_booking
        from guests
        join bookings on guests.guest_id = bookings.guest_id 
        group by guests.guest_id ;
        
select r.room_type, sum(datediff(check_out, check_in) * r.price_per_day) as total_money
from bookings b
